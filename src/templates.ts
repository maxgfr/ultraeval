...
// The sequential eco fallback: the SAME stage order the generated workflow runs,
// rendered as a self-pass checklist over the emitted agents/*.md contracts.
export function runbookMd(cfg: EvalConfig, runDirAbs: string, engineAbs: string): string {
  const mode = cfg.mode ?? "audit";
  const doDefects = mode !== "improve";
  const doOpps = mode !== "audit";
  const stages: { title: string; contract: string; note: string }[] = [];
  ...
  for (const [i, lens] of ["correctness+grounding", "completeness+coverage", "ux+meets-expectations"].entries())
    stages.push({ title: `Judge ${i + 1}`, contract: "judge", note: `LENS=${lens}; calibrate first, append your verdict line to judges.jsonl.` });
