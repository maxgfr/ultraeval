...
**Manual by default.** `ultraeval` runs when you ask for it: `$ultraeval` in Codex,
`/ultraeval` in Claude Code or OpenCode. The agent never starts it on its own, and
...
`ultraeval` ships **explicit-only**, and `skills add` installs it that way: it runs
when you invoke it, never when the agent feels like it. Use `$ultraeval` in Codex,
`/ultraeval` in Claude Code or OpenCode, prefixing the plugin namespace when it is
...
`"skillOverrides": { "ultraeval": "on" }` in `settings.json`, where
...
skill-tool loading, while the explicit `/ultraeval` command still works.
