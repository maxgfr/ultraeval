#!/usr/bin/env node
// Sync the release version across the three places it lives, then let the caller
...
// src/types.ts — the VERSION constant the engine/bundle reports.
edit("src/types.ts", (s) => s.replace(/(export const VERSION = ")[^"]+(";)/, `$1${version}$2`));

// skills/ultraeval/SKILL.md — the indented `version:` under the frontmatter
// `metadata:` block. The skill is packaged under skills/ultraeval/ so that
// `npx skills add` bundles the engine + references with it.
edit("skills/ultraeval/SKILL.md", (s) => s.replace(/(\n[ \t]+version:[ \t]*)[^\n]+/, `$1${version}`));

console.log(`sync-version: set ${version} in package.json, src/types.ts, skills/ultraeval/SKILL.md`);
