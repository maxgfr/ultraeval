...
beforeAll(async () => {
  const base = mkdtempSync(join(tmpdir(), "ue-http-"));
  temps.push(base);
  const target = join(base, "target");
  RUN = join(base, "run");
  mkdirSync(join(target, "src"), { recursive: true });
  writeFileSync(join(target, "package.json"), JSON.stringify({ name: "target", version: "1.0.0" }));
  writeFileSync(join(target, "src", "index.js"), "module.exports = { add: (a, b) => a + b };" + String.fromCharCode(10));
...
describe("the JSON-RPC contract", () => {
  it("initializes and identifies itself, issuing no session id", async () => {
    ...
    expect(body.result.serverInfo.name).toBe("ultraeval");
  });
  ...
  it("lists tools, and answers a batch with an array", async () => {
    ...
    expect(single.json().result.tools.map((t: { name: string }) => t.name)).toContain("ultraeval_status");
  ...
  it("runs a real tool call end to end", async () => {
    const res = await post({ body: { jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "ultraeval_status", arguments: { run: RUN } } } });
    expect(res.status).toBe(200);
    expect(JSON.parse(res.json().result.content[0].text).run).toBe(RUN);
  });
  ...
  it("returns a failing tool as 200 + isError, never as an HTTP 5xx", async () => {
    ...
    const res = await post({
      body: { jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "ultraeval_status", arguments: { run: "/nope/not/a/run" } } },
    });
    expect(res.status).toBe(200);
    const result = res.json().result;
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toMatch(/no evaluation run/);
  });
});
...
describe("statelessness under concurrency", () => {
  it("keeps two overlapping requests on different protocol versions from reading each other's", async () => {
    ...
    const pick = (r: { json: () => { result: { tools: { name: string }[] } } }) =>
      r.json().result.tools.find((t) => t.name === "ultraeval_read") as Record<string, unknown>;
...
describe("protocol version negotiation over HTTP", () => {
  it("accepts a supported version header", async () => {
    ...
    const read = res.json().result.tools.find((t: { name: string }) => t.name === "ultraeval_read");
