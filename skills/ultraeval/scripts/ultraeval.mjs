...
function collectCallsRegex(content) {
  ...
  for (let i2 = 0; i2 < lines.length && out2.size < 512; i2++) {
    ...
    while ((m = CALL_RE.exec(line)) !== null && out2.size < 512) {
      ...
      if (!out2.has(key2)) out2.set(key2, { name: name2, line: i2 + 1 });
    }
  }
...
function tomlStringArray(body2, key2) {
  const m = body2.match(new RegExp(`${key2}\\s*=\\s*\\[([^\\]]*)\\]`));
  if (!m) return [];
...
function npmFamilyPatterns(root) {
  ...
  const pkg = readJson2(join5(root, "package.json"));
  ...
  const pnpm = readText2(join5(root, "pnpm-workspace.yaml"));
...
function detectCargoMembers(root, found) {
  const toml = readText2(join5(root, "Cargo.toml"));
  if (!toml) return;
...
function detectGoWork(root, found) {
  const gowork = readText2(join5(root, "go.work"));
  if (!gowork) return;
  const dirs = [];
...
function detectMavenModules(root, found) {
  const pom = readText2(join5(root, "pom.xml"));
  if (!pom) return;
...
function npmEdges(root, pkg, byName) {
  const manifest = readJson2(join5(root, pkg.dir, "package.json"));
  if (!manifest) return [];
...
function cargoEdges(root, pkg, byName, byDir) {
  const toml = readText2(join5(root, pkg.dir, "Cargo.toml"));
  if (!toml) return [];
  const edges = /* @__PURE__ */ new Set();
...
function goPkgEdges(root, pkg, byName, byDir) {
  const gomod = readText2(join5(root, pkg.dir, "go.mod"));
  if (!gomod) return [];
  const edges = /* @__PURE__ */ new Set();
...
function mavenEdges(root, pkg, byName) {
  const pom = readText2(join5(root, pkg.dir, "pom.xml"));
  if (!pom) return [];
  const edges = /* @__PURE__ */ new Set();
...
function emit(content, out2) {
  if (out2) writeFileSync2(out2, content);
...
async function runCli(argv) {
  ...
  if (cmd === "index") {
    ...
    writeFileSync2(join6(outDir, "graph.json"), renderGraphJson(graph));
    writeFileSync2(join6(outDir, "symbols.json"), renderSymbolsJson(symbols));
    const files = {};
    for (const f of scan2.files) {
...
function walkFiles(root) {
  ...
  for (const rec of scan2.files) {
    ...
    const content = readText2(join7(root, rec.rel));
    const resolved = [];
...
function analyzeRepo(targetAbs, opts = {}) {
  ...
  const docs = ["README.md", "DOCUMENTATION.md", "CONTRIBUTING.md", "docs"].filter((d) => exists(join7(targetAbs, d)));
  return {
    target: targetAbs,
    files: files.length,
...
function runAnalyze(targetDir, outDir, opts = {}) {
  ...
  writeJson(join7(outDir, "analysis.json"), a);
  writeText(join7(outDir, "ANALYSIS.md"), renderAnalysisMd(a));
  return a;
}
...
function freshTestFile(guess, disc, targetAbs) {
  if (!exists(join8(targetAbs, guess))) return guess;
  let candidate = distinctTestPath(guess, disc);
  for (let n = 2; exists(join8(targetAbs, candidate)); n++) candidate = distinctTestPath(guess, `${disc}-${n}`);
  return candidate;
}
function detectVerifyCommand(targetAbs) {
  if (exists(join8(targetAbs, "package.json"))) {
    const pkg = readJson(join8(targetAbs, "package.json"));
    if (pkg.scripts?.test) {
      if (exists(join8(targetAbs, "pnpm-lock.yaml"))) return "pnpm test";
      if (exists(join8(targetAbs, "yarn.lock"))) return "yarn test";
      return "npm test";
    }
  }
  if (exists(join8(targetAbs, "go.mod"))) return "go test ./...";
  if (exists(join8(targetAbs, "Cargo.toml"))) return "cargo test";
  if (exists(join8(targetAbs, "pytest.ini")) || exists(join8(targetAbs, "pyproject.toml"))) return "pytest";
  return null;
}
function buildBacklog(runDir, opts = {}) {
  ...
  const vpath = join8(runDir, "VERIFY.json");
  if (exists(vpath)) {
  ...
  const tasks = confirmed.map((f, i2) => {
    ...
    return {
      ...
      red: {
        testFile,
        expectedNew: !exists(join8(targetAbs, testFile)),
  ...
  writeJson(join8(out2, "BACKLOG.json"), backlog);
  writeText(join8(out2, "REMEDIATION.md"), renderRemediation(backlog, cfg));
  if (opts.tdd) {
    const byId = new Map(doc.findings.map((f) => [f.id, f]));
    for (const t of tasks) writeText(join8(out2, "fixes", `${t.id}-${slug(t.title)}.md`), renderFixCard(t, byId.get(t.findingId)));
  }
  return backlog;
}
...
function runBrainstorm(runDir) {
  ...
  const analysis = exists(join9(runDir, "analysis.json")) ? readJson(join9(runDir, "analysis.json")) : null;
  writeText(join9(runDir, "BRAINSTORM.todo.md"), renderTodo(cfg, analysis));
...
function rankBrainstorm(runDir) {
  const oppsPath = join9(runDir, "opportunities.json");
  ...
  const doc = exists(join9(runDir, "findings.json")) ? readJson(join9(runDir, "findings.json")) : { findings: [] };
  let maxN = 0;
  ...
  writeJson(join9(runDir, "findings.json"), doc);
...
import { join as join10, relative as relative3, sep as sep2 } from "path";
...
function ensureGitignored(runDirAbs) {
  ...
  const existing = existsSync4(path) ? readText(path) : "";
...
function detectKind(targetAbs) {
  if (exists(join11(targetAbs, "SKILL.md"))) return "skill";
  const skillsDir = join11(targetAbs, "skills");
  if (exists(skillsDir)) {
...
function initRun(opts) {
  ...
  ensureDir(join11(runDir, "runs"));
  ensureDir(join11(runDir, "research"));
  writeJson(join11(runDir, "eval.config.json"), cfg);
...
import { isAbsolute as isAbsolute3, join as join12, resolve as resolve4 } from "path";
...
function runVerify(runDir, opts = {}) {
  ...
  writeJson(join12(runDir, sh2 ? `VERIFY.todo.${opts.shard}.json` : "VERIFY.todo.json"), out2);
  writeText(join12(runDir, sh2 ? `VERIFY.${opts.shard}.md` : "VERIFY.md"), renderWorklistMd(out2));
...
function plantHoneypots(runDir, pairs, n, shard) {
  ...
  if (traps.length) {
    ...
    writeJson(join12(runDir, truthName), {
...
function loadHoneypotIds(runDir) {
  const ids = /* @__PURE__ */ new Set();
  const files = [join12(runDir, "VERIFY.honeypots.json")];
  if (exists(runDir)) {
    for (const e of readdirSync4(runDir)) if (/^VERIFY\.honeypots\.\d+\.json$/.test(e)) files.push(join12(runDir, e));
  }
  for (const f of files) {
...
function checkRun(runDir, opts = {}) {
  const errors = [];
  const warnings = [];
  const cfgPath = join13(runDir, "eval.config.json");
  if (!exists(cfgPath)) {
  ...
  const findingsPath = join13(runDir, "findings.json");
  ...
  const backlogPath = join13(runDir, "BACKLOG.json");
  ...
  const verifyPath = join13(runDir, "VERIFY.json");
  ...
  if (exists(join13(runDir, "RESULTS.md")) && !exists(join13(runDir, "SUMMARY.md"))) warnings.push("RESULTS.md present but no SUMMARY.md");
  if (exists(join13(runDir, "runs", "budget.md"))) {
    const summaryPath = join13(runDir, "SUMMARY.md");
...
function load(dir) {
  const findings = exists(join14(dir, "findings.json")) ? readJson(join14(dir, "findings.json")).findings ?? [] : [];
  const score = exists(join14(dir, "scorecard.json")) ? readJson(join14(dir, "scorecard.json")) : null;
  const cfg = exists(join14(dir, "eval.config.json")) ? readJson(join14(dir, "eval.config.json")) : null;
...
function runCompare(baseDir, newDir, outDir) {
  ...
  writeText(join14(outDir, "COMPARE.md"), r.md);
  return r;
}

// src/sarif.ts
import { join as join15, relative as relative4, sep as sep3 } from "path";
...
function writeSarif(runDir, out2) {
  ...
  const p = join15(out2 ?? runDir, "eval.sarif");
...
function clean(runDir, opts = {}) {
  const removed = [];
  if (exists(runDir) && !exists(join16(runDir, "eval.config.json"))) {
  ...
  if (exists(runDir)) {
    for (const e of readdirSync5(runDir)) {
      if (/^VERIFY\.(todo\.|honeypots\.)?\d+\.(json|md)$/.test(e)) {
        rmSync2(join16(runDir, e), { force: true });
        removed.push(join16(runDir, e));
      }
    }
  }
  return removed;
...
function calibrationFixturePath(engineAbs) {
  const candidates = [
    join17(dirname3(engineAbs), "..", "references", "calibration-run.json"),
    join17(dirname3(engineAbs), "..", "skills", "ultraeval", "references", "calibration-run.json")
  ];
...
function oneshotRun(opts, engineAbs) {
  ...
  const written = [join18(runDir, "eval.config.json")];
  const w = (rel, content) => {
    const p = join18(runDir, rel);
    writeText(p, content);
    written.push(p);
  };
...
function planRun(runDir, engineAbs, opts = {}) {
  let cfg = readJson(join19(runDir, "eval.config.json"));
  const written = [];
  const w = (rel, content) => {
    const p = join19(runDir, rel);
    writeText(p, content);
    written.push(p);
  };
  if (cfg.oneshot || cfg.provenance?.profile) {
    rmSync3(join19(runDir, "ONESHOT.md"), { force: true });
    ...
    written.push(join19(runDir, "eval.config.json"));
  }
  if (opts.eco === true) {
    rmSync3(join19(runDir, "eval.workflow.mjs"), { force: true });
    w("RUNBOOK.md", runbookMd(cfg, runDir, engineAbs));
  } else {
    rmSync3(join19(runDir, "RUNBOOK.md"), { force: true });
...
import { isAbsolute as isAbsolute4, join as join20, resolve as resolve5 } from "path";
function loadBacklog(runDir) {
  const blPath = join20(runDir, "BACKLOG.json");
  if (!exists(blPath)) throw new Error("no BACKLOG.json \u2014 run `backlog --run <run> --tdd` first, then fix");
  return { cfg: readJson(join20(runDir, "eval.config.json")), backlog: readJson(blPath) };
}
function targetInvariants(targetAbs) {
  const lines = [];
  const pkgPath = join20(targetAbs, "package.json");
  if (exists(pkgPath)) {
...
function agentContract(t, cfg, runDir, engineAbs) {
  const targetAbs = resolveTargetAbs(cfg.targetAbs, cfg.target, runDir);
  const absTargets = t.targets.map((x) => isAbsolute4(x) ? x : join20(targetAbs, x));
  const redFile = isAbsolute4(t.red.testFile) ? t.red.testFile : join20(targetAbs, t.red.testFile);
  const deps = t.dependsOn.length ? `
Depends on: ${t.dependsOn.join(", ")} \u2014 confirm each is \`status: "done"\` in \`${join20(runDir, "BACKLOG.json")}\` before starting; if not, STOP and report.` : "";
...
function emitFixAgents(runDir, engineAbs, opts = {}) {
  ...
  const written = [];
  for (const t of tasks) {
    const p = join20(runDir, "fixes", "agents", `${t.id}.agent.md`);
    writeText(p, agentContract(t, cfg, runDir, engineAbs));
    written.push(p);
  }
  if (opts.workflow) {
    const p = join20(runDir, "fix.workflow.mjs");
...
function verifyFix(runDir, taskId, opts = {}) {
  ...
  if (ok) {
    ...
    writeJson(join20(runDir, "BACKLOG.json"), backlog);
  }
  return result;
}
...
function rejudgeRun(runDir, outDir, engineAbs) {
  if (!exists(join21(runDir, "eval.config.json"))) throw new Error(`refusing to rejudge ${runDir}: not an ultraeval run (no eval.config.json)`);
  const cfg = readJson(join21(runDir, "eval.config.json"));
  mkdirSync3(outDir, { recursive: true });
  const copied = [];
  for (const f of COPY_FILES) {
    if (!exists(join21(runDir, f))) continue;
    cpSync(join21(runDir, f), join21(outDir, f));
    copied.push(f);
  }
  for (const d of COPY_DIRS) {
    if (!exists(join21(runDir, d))) continue;
    cpSync(join21(runDir, d), join21(outDir, d), { recursive: true });
    copied.push(`${d}/`);
  }
  writeText(join21(outDir, "judges.jsonl"), "");
  writeText(join21(outDir, "agents", "judge.md"), agentContracts(cfg, outDir, engineAbs).judge);
  writeText(join21(outDir, "rejudge.workflow.mjs"), rejudgeWorkflow(cfg, outDir, engineAbs, runDir));
  return copied;
}
...
function load2(runDir) {
  ...
  const verify = exists(join23(runDir, "VERIFY.json")) ? readJson(join23(runDir, "VERIFY.json")) : null;
  const backlog = exists(join23(runDir, "BACKLOG.json")) ? readJson(join23(runDir, "BACKLOG.json")) : null;
  const scorecard = exists(join23(runDir, "scorecard.json")) ? readJson(join23(runDir, "scorecard.json")) : null;
...
function render(runDir, opts = {}) {
  ...
  const written = [];
  if (opts.md !== false) {
    const p = join23(out2, "index.md");
    writeText(p, buildMd(cfg, doc, verify, backlog, scorecard));
    written.push(p);
  }
  if (opts.html !== false) {
    const p = join23(out2, "index.html");
...
function readJudges(runDir) {
  const p = join24(runDir, "judges.jsonl");
  if (!exists(p)) return [];
...
function scoreRun(runDir) {
  ...
  writeJson(join24(runDir, "scorecard.json"), sc);
  return sc;
}
function appendHistory(runDir, file) {
  const scPath = join24(runDir, "scorecard.json");
...
function defaultLedgerPath(runDir) {
  ...
  return join24(base, "evals", "history.jsonl");
}
...
function statusRun(runDir) {
  const has = (rel) => exists(join25(runDir, rel));
  let oneshot = false;
  try {
    if (has("eval.config.json")) oneshot = readJson(join25(runDir, "eval.config.json")).oneshot === true;
  } catch {
    oneshot = false;
  }
  if (oneshot) {
  ...
  const judgesPresent = has("judges.jsonl") && readText(join25(runDir, "judges.jsonl")).trim().length > 0;
  const steps = [
    { artifact: "eval.config.json", present: has("eval.config.json"), stage: "init" },
...
function cmdAnalyze(args2) {
  ...
  let out2;
  if (run2) {
    const cfg = readJson(join26(run2, "eval.config.json"));
...
function cmdCheck(args2) {
  ...
  if (!exists(join26(run2, "eval.config.json"))) throw new Error(`no eval.config.json under ${run2} \u2014 not an ultraeval run; run \`ultraeval init\` first`);
