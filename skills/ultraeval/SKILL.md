...
`Workflow({ scriptPath: "<RUN>/eval.workflow.mjs" })`. It pipelines **Research → TestPlan → Execute → Findings → Gate → Judge → Results** and self-invokes the engine gates. On Codex, use its native subagent capability to dispatch the contracts under `<RUN>/agents/`; on any host without subagents, follow `<RUN>/RUNBOOK.md` sequentially. **The stage name is not always the file name** — the Results stage reads `<RUN>/agents/remediator.md`. See `references/orchestration.md`. **Every subagent gets the ABSOLUTE `<skill-dir>/scripts/ultraeval.mjs` path** — it has its own cwd and cannot resolve a relative one (`plan` already bakes the absolute path into the workflow).
